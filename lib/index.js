...
class SearchScraper {
  ...
  static register(c) {
    if (this._scrapers[c.name]) {
      console.warn(
        `There was already registered a scraper with this name: ${c.name}`
      );
    }
  ...
  async search(term, _options) {
    ...
    if (proxyAuth) await browser.authenticate(proxyAuth).catch(() => {});
    ...
    const acceptTerms = async () => {
      ...
      if (options.selectors.accept_terms_button) {
        if (typeof options.selectors.accept_terms_button === "function") {
          ...
          try {
            await page.click(options.selectors.accept_terms_button, {
              delay: 20,
            });
            await sleep(1000);
            await page.click(options.selectors.accept_terms_button, {
    ...
    const doSync = async () => {
      ...
      try {
        ...
        this.log(
          "Did not get the results after waiting. Perhaps there are no results returned by the search engine;."
        );
        return;
      }
      ...
      let newLinks = await page.evaluate(
        options.scrape_fn ||
          ((opts) => {
            ...
            return [].slice
              .apply(document.querySelectorAll(resultSelector))
              .map(function (c) {
                var newItem = {
                  title: (
                  ...
                  description: (
                    (descriptionSelector
                      ? c.querySelector(descriptionSelector)
                      : c) || { textContent: null }
                  ).textContent,
                };
                c.remove();
      ...
      options.selectors.result.filter =
        options.selectors.result.filter ||
        ((c) => c.title !== null && /^https?:\/\//.test(c.url));
      ...
      allLinks = Object.values(
        allLinks.reduce((acc, c) => {
          acc[c.url] = c;
          return acc;
        }, {})
      );
...
const clickAcceptAllBtn = () => {
  const btns = [].slice
    ...
    .filter((c) => /accept/i.test(c.textContent));
